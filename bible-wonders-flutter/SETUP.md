...
The native projects come with the clone, so there is no `flutter create` step.
Generate the two assets, fetch the packages, and go:
...
- **The tour** (`features/tour/`) walks the ranked shortlist, not the curated
  ...
  read it here. Narration itself is done — the tour reads each step, and will
  read them automatically if asked.
